...
#ifdef LOG_CUSTOM_MSG
#undef LOG_CUSTOM_MSG
#endif
#ifdef LOG_CUSTOM_MSG2
...
/*
        ...
        bool destroy_thread(Queue* queue, int64_t& busy_count)
        {
            for (auto &q : queues_) {
                if (q->destroy_thread(queue, busy_count)) return true;
            }
            return false;
        }
*/
        ...
        // ----------------------------------------------------------------
        std::size_t get_thread_count(thread_state_enum state = unknown) const
        {
            std::size_t len = 0;
        ...
        public:
            ...
            shared_priority_scheduler(std::size_t num_worker_queues,
              ...
              , num_workers_(num_worker_queues)
              , num_domains_(1)
              , hp_numa_(num_high_priority_queues_)
              , initialized_(false)
            {
...
bool cleanup_terminated(std::size_t num_thread, bool delete_all) {
    return cleanup_terminated(delete_all);
                ...
                thread_schedule_hint schedulehint)
            {
                ...
                {
                    //std::cout << "create_thread thread_schedule_hint_none" << std::endl;
...
//                    std::cout << "create_thread HINT " << domain_num << std::endl;
                }
                    ...
                    domain_num = d_lookup_[pool_queue_num];
                    q_index    = q_lookup_[pool_queue_num];
//                    std::cout << "create_thread NORMAL " << domain_num << std::endl;
                }

                LOG_CUSTOM_MSG("create_thread " << " queue "
...
//                                                  << decnumber(pool_queue_num));
                ...
                if (result)
                {
                    ...
                    LOG_CUSTOM_MSG("got next thread " << " queue "
                                   << "queue " << decnumber(pool_queue_num)
                                   << "domain " << decnumber(domain_num));
                }
/*
...
*/
                ...
                thread_priority priority = thread_priority_normal)
            {
                ...
                //
                LOG_CUSTOM_MSG("schedule_thread " << " queue "
                                                  << decnumber(pool_queue_num));
                //
                ...
                LOG_CUSTOM_MSG2("thread scheduled on "
                               << "queue " << decnumber(pool_queue_num)
                               << "domain " << decnumber(domain_num)
                               << "qindex " << decnumber(q_index));
...
//                              << std::dec << domain_num << std::endl;
            ...
            // Destroy the passed thread - as it has been terminated
            //---------------------------------------------------------------------
            bool destroy_thread(threads::thread_data* thrd, std::size_t pool_queue_num,
                         std::int64_t& busy_count)
            {
                ...
                // the correct queue to use for destruction was stored during creation
                // so retrieve it and use it here
                thread_queue_type* queue
                    = reinterpret_cast<thread_queue_type*>(thrd->get_queue());
                return queue->destroy_thread(thrd, busy_count);
            }
                ...
                std::size_t pool_queue_num = std::size_t(-1)) const
            {
                ...
                if (pool_queue_num != std::size_t(-1)) {
                    // @ TODO : Return queue length of one specific queue.
                    throw std::runtime_error("Implement get_queue_length");
                }
                return count;
            }
            ...
            void on_start_thread(std::size_t pool_queue_num)
            {
                ...
                if (!initialized_)
                {
                    ...
                    for (std::size_t i=0; i<num_domains_; ++i) {
                        // high priority - do not use more than q count
                        hp_queues_[i].init(
                            q_counts_[i],
                            std::max(std::min(hp_numa_, q_counts_[i]), std::size_t(1)),
                            max_queue_thread_count_);
                        // normal priority
                        np_queues_[i].init(
                            q_counts_[i], q_counts_[i], max_queue_thread_count_);
                        // low oriority
                        lp_queues_[i].init(
                            q_counts_[i], 1, max_queue_thread_count_);
                    }
                }
            }
...
//                    << decnumber(pool_queue_num));
            ...
            //---------------------------------------------------------------------
            //---------------------------------------------------------------------
            void on_error(std::size_t pool_queue_num, std::exception_ptr const& e)
            {
                throw std::runtime_error(
                            "on_error has never been called");
            }
            ...
            {
                throw std::runtime_error(
                            "reset_thread_distribution has never been called");
            }
